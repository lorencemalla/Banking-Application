...
import { RiBankLine, RiExchangeFundsLine, RiBillLine, RiBankCardLine, RiHandCoinLine, RiFileListLine, RiCustomerService2Line, RiUser3Line } from 'react-icons/ri';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [accRes, txRes] = await Promise.all([accountAPI.getAll(), transactionAPI.getHistory()]);
      ...
      <div className="grid-2" style={{ marginBottom: 24 }}>
        ...
        <div className="glass-card">
          ...
          {transactions.length === 0 ? (
            ...
            transactions.map(tx => (
              <div key={tx.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid var(--border-light)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 36, height: 36, borderRadius: '50%', background: tx.direction === 'CREDIT' ? 'var(--green-light)' : 'var(--red-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: tx.direction === 'CREDIT' ? 'var(--green)' : 'var(--red)', fontSize: '0.9rem' }}>
                    {tx.direction === 'CREDIT' ? '↓' : '↑'}
                  </div>
                  <div>
                    <div style={{ fontWeight: 500, fontSize: '0.85rem', color: 'var(--text-dark)' }}>{tx.description || tx.type}</div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{new Date(tx.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</div>
                  </div>
                </div>
                <div className={tx.direction === 'CREDIT' ? 'amount-credit' : 'amount-debit'} style={{ fontSize: '0.9rem' }}>
